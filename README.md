...
Files marked as generated by `.gitattributes` via `linguist-generated=true` are excluded from
scoring entirely. Later `linguist-generated=false` overrides are respected.
...
If `--repo` points at a repository with `.gitattributes`, generated files from that repository
are excluded in patch mode as well.
...
`gitattributes_skip_attributes` lists `.gitattributes` attribute names that should cause a file to be excluded from scoring.
